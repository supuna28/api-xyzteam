...
try {
var zahirr = db.get("thisleonreal");
} catch (e) {
  console.log('')
}

var creatorList = ['@thisleonreal','@lionchou'];
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------thisleonreal'+'LION--CHOU';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'xyz') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
  ...
  if(!apikeyInput) return res.json(loghandler.notparam)
  if(apikeyInput != 'xyz') return res.json(loghandler.invalidKey)
